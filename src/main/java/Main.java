...
public class Main {
    // Utilisation de l'interface pour le couplage faible
    private static IMedicamentDao dao = new MedicamentDaoImpl();
    private static Scanner scanner = new Scanner(System.in);

    public static void main(String[] args) {
        // Données de test pour ne pas partir de zéro
        initialiserDonneesTests();
    ...
    private static void afficherMenuPrincipal() {
        ...
        System.out.println("3. Rechercher un médicament (par code)");
    ...
    private static void menuAjouter() {
        ...
        switch (type) {
            case 1: // Comprime
                ...
                int nb = lireEntier("Nombre de comprimés par boite : ");
        ...
        if (nouveau != null) {
            dao.ajouter(nouveau);
            System.out.println("Succès : Médicament ajouté avec succès !");
        }
    }
    ...
    private static void menuRechercher() {
        ...
        if (m != null) {
            ...
            System.out.println("Aucun médicament trouvé avec ce code.");
        }
    }

    private static void menuMiseAJourStock() {
        System.out.print("\nEntrez le code du médicament à modifier : ");
        String code = scanner.nextLine();
        Medicament m = dao.trouverParCode(code);
        ...
        if (m != null) {
            ...
            System.out.println("Succès : Stock mis à jour.");
        } else {
    ...
    private static void menuSupprimer() {
        System.out.print("\nEntrez le code du médicament à supprimer : ");
        String code = scanner.nextLine();
        System.out.print("Êtes-vous sûr ? (o/n) : ");
        String confirm = scanner.nextLine();

        if (confirm.equalsIgnoreCase("o")) {
            boolean sup = dao.supprimer(code);
            if (sup) System.out.println("Succès : Médicament supprimé.");
            else System.out.println("Erreur : Code introuvable.");
        } else {
            System.out.println("Suppression annulée.");
        }
    }

    // --- UTILITAIRES POUR LES ENTREES UTILISATEUR ---
    // Ces méthodes évitent le bug du scanner qui saute une ligne et gèrent les erreurs de type
    ...
    private static double lireDouble(String message) {
        while (true) {
            try {
                ...
                System.out.println("Erreur : Veuillez entrer un prix valide (ex: 12.5).");
            }
        }
    }

    private static void initialiserDonneesTests() {
        dao.ajouter(new Comprime("Paracetamol", "DOL100", 5.5, "12/2026", 50, "Sanofi", "500mg", 16));
        dao.ajouter(new Sirop("Toplexil", "TOP200", 8.2, "05/2025", 20, "Sanofi", 150, "Caramel"));
    }
}